function FineCodeForecast(options) {
	...
	var _BaseUrl = _ctxPath+'/forecast/fineCodeForecast';
		...
		columns : [dataSelModel, {
			...
			renderer : function(value, meta, record, cellmeta) {
				...
				resultStr = "<div class='controlBtn'>"
						...
						+ record.id
						+ ");' class='a' onclick='FineCodeForecast.showViewWindow()'>预览</a>&nbsp;&nbsp;&nbsp;&nbsp;"
				"</div>";
				return resultStr;
			}
		}],
	...
	function _buildCodeFields(days) {
		...
		for(var i=1;i<=days;i++) {
			fields.push({
				name: 'phenomena{DAY}a'.replace('{DAY}', i)
			}, {
				name: 'phenomena{DAY}b'.replace('{DAY}', i)
			}, {
				name: 'phenomena{DAY}c'.replace('{DAY}', i)
			}, {
				name: 'phenomena{DAY}d'.replace('{DAY}', i)
			}, {
				name: 'windDir{DAY}a'.replace('{DAY}', i)
			}, {
				name: 'windDir{DAY}b'.replace('{DAY}', i)
			}, {
				name: 'windDir{DAY}c'.replace('{DAY}', i)
			}, {
				name: 'windDir{DAY}d'.replace('{DAY}', i)
			}, {
				name: 'windPow{DAY}a'.replace('{DAY}', i)
			}, {
				name: 'windPow{DAY}b'.replace('{DAY}', i)
			}, {
				name: 'windPow{DAY}c'.replace('{DAY}', i)
			}, {
				name: 'windPow{DAY}d'.replace('{DAY}', i)
			}, {
				name: 'temp{DAY}a'.replace('{DAY}', i)
			}, {
				name: 'temp{DAY}b'.replace('{DAY}', i)
			}, {
				name: 'temp{DAY}c'.replace('{DAY}', i)
			}, {
				name: 'temp{DAY}d'.replace('{DAY}', i)
			});
		}
		return fields;
	}
	...
	function _buildCodeGridColumns(days) {
		...
		for(var i=1;i<=days;i++) {
			...
			for(var n=0;n<spanLen;n++) {
				var col = {
					header : '天气现象{H}小时'.replace('{H}', hour-(spanLen-n-1)*span),
					width : 100,
					dataIndex : 'phenomena{DAY}{S}'.replace('{DAY}',i).replace('{S}', String.fromCharCode(97+n))	
				};
				if(_EnableEdit) {
			...
			for(var n=0;n<spanLen;n++) {
				col = {
					header : '风向{H}小时'.replace('{H}', hour-(spanLen-n-1)*span),
					width : 80,
					dataIndex : 'windDir{DAY}{S}'.replace('{DAY}', i).replace('{S}', String.fromCharCode(97+n))
				};
				if(_EnableEdit) {
				...
				columns.push(col);
				col = {
					header : '风力{H}小时'.replace('{H}', hour-(spanLen-n-1)*span),
					width : 80,
					dataIndex : 'windPow{DAY}{S}'.replace('{DAY}', i).replace('{S}', String.fromCharCode(97+n))
				};
				if(_EnableEdit) {
			...
			for(var n=0;n<spanLen;n++) {
				columns.push({
					header : '温度{H}小时'.replace('{H}', hour-(spanLen-n-1)*span),
					width : 90,
	...
	//							accelerate : true
	//						})),
					dataIndex : 'temp{DAY}{S}'.replace('{DAY}', i).replace('{S}', String.fromCharCode(97+n)),
					renderer0: function(value, meta, record) {
						var resultStr = ["<div class='controlBtn'>", value, "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;",
								"<a href='javascript:void(0);' class='a' onclick='FineCodeForecast.showTempWindow({H})'>选择</a>".replace('{H}', hour-12),
								"&nbsp;&nbsp;&nbsp;&nbsp;</div>"].join('');
						return resultStr;
					}
				});
	...
	function buildForecast() {
		loadAjaxData({
			url: _ctxPath+'/forecast/fineCodeForecast/gridToForecast',
	...
	// 预览电码
	var showPreviewWindow = FineCodeForecast.showViewWindow = function(options) {
...
//									})),
	...
	// 显示温度弹出框
	FineCodeForecast.showTempWindow = function(time) {
		// 全局变量，用于时间选择
