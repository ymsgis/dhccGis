...
<body title="最小相对湿度单时效检验" style="padding: 0; margin: 0;">
				...
				<td style="width: 100%">
					...
					<div id="tb" style="padding: 3px">
						...
						<select id="time" class="easyui-combobox" 
							name="time" style="width: 60px;">
								<option value="eight">8</option>
								<option value="twenty">20</option>
						</select>
						<span>时效：</span> 
						<select id="aging" name="aging" value="24" class="easyui-combobox" style="width: 50px;">
                    		<c:forEach begin="24" end="240" step="24" var="idx">
                        		<option>${idx}</option>
                    		</c:forEach>
                		</select>
						<span>预报类型：</span>
						...
						<select id="forecaster" class="easyui-combobox" 
							name="forecaster" style="width: 80px;">
								<option value="#"></option>
<!-- 								<option value="#1">预报员</option> -->
<!-- 								<option value="#2">预报员</option> -->
						</select>
...
function doSearch(){
	...
	$('#tt').datagrid({
		url: "${contextPath}/check/checkErhi/normal/loadList",
		queryParams: queryParams
	});
	window.parent.erhi.normalChart(queryParams);
}
$(document).ready(function(){
    ...
    $('#tt').datagrid({
        ...
        columns:[[
            ...
            {field:'sst', title:'技巧评分', width:80, hidden:'true',
                formatter: function(value,row,index){
                    if (value!=null){
                        if(9999 != value){
            ...
            {field:'averageError', title:'平均误差', width:80,
                formatter: function(value,row,index){
                    if (value!=null){
                        return value;
                    } else {
            ...
            {field:'rmsError', title:'均方根误差', width:90,
                formatter: function(value,row,index){
                    if (value!=null){
                        return value;
                    } else {
            ...
            {field:'tenDegreesAccuracy', title:'≤10%准确率', width:90,
                formatter: function(value,row,index){
                    if (value!=null){
                        return value;
                    } else {
